...
#Hay que modificar el EntangleNodes para que soporte bien las posiciones de memoria extra necesarias para el Distil,
#creo que los problemas van a venir por ahi. La posición extra que usas para destilar ha de estar conectada tambien con el otro nodo.
        ...
        while True:
            ...
            for mem_pos in self._mem_positions[::-1]:
                ...
                if mem_pos != self._input_mem_pos: 
                    if 'end_node_0' in self.node.name:#esta ultima restriccion la he puesto a mano para mantener la estructura
                        mem_pos=2 
                    elif 'end_node_1' in self.node.name:
                        mem_pos= 3                      
    ...
    def run(self):
        ...
        if END_NODE_BASENAME in self.nodelist[-1].name:
            while True:
                yield(self.start_expression)
                # Perform Bell measurement
                routing= FidelityObtainer(nodes=self.nodelist, name="Channel_Decider_Last")
                if self.purify == 'filter':
                    secpos, firstpos = routing.PositionDecision()
                else:
                    secpos, firstpos = 3, 2
                yield self.node.qmemory.execute_program(self._program, qubit_mapping=[secpos, firstpos])
                m = [self._program.output["m"], 1, 0]

                # Send result to right node on end
                self.node.ports["ccon_R"].tx_output(Message(m, header='swapend'))
                self.node.ports["ccon_L3"].tx_output(Message(m, header='swapend'))
        ...
        if SWITCH_NODE_BASENAME in self.nodelist[-1].name:
            while True:
                ...
                if evt_expr_wait.first_term.value and evt_expr_wait.second_term.value:
                    ...
                    message1 = self.node.ports["ccon_R3"].rx_input()
                #ic('He leído mensajes en SWAP2')
                #if message1 is None or message2 is None:
                #    self.protocol.send_signal(Signals.WAITING)
                #else:
...
class CorrectProtocol(NodeProtocol):
    ...
    def __init__(self, node, num_nodes, purify):
        ...
        while True:
            ...
            if self._counter == self.num_nodes - 2:
                if self._x_corr or self._z_corr:
                    ...
                    position_a=3 if self.purify == 'distil' else final_secpos
...
def setup_network(num_nodes, node_distance, source_frequency):
    ...
    for i in range(num_nodes - 1):
        ...
        qconn2 = EntanglingConnection(name=f"qconn_{i}-{i+1}2", length=node_distance,
                                     source_frequency=source_frequency)
        # Add a noise model which depolarizes the qubits exponentially
        # depending on the connection length
        for channel_name in ['qchannel_C2A', 'qchannel_C2B']:#, 'qchannel_C2A1', 'qchannel_C2B1']:
            qconn.subcomponents[channel_name].models['quantum_noise_model'] = FibreDepolarizeModel(p_depol_init=conf['p_depol_init'] - 0.25*uniform(0,1),
                                                                                                   p_depol_length=conf['p_depol_length'])
            qconn2.subcomponents[channel_name].models['quantum_noise_model'] = FibreDepolarizeModel(p_depol_init=conf['p_depol_init']+ 0.25*uniform(0,1),
                                                                                                    p_depol_length=conf['p_depol_length'])
        ...
        port_name2, port_r_name2 = network.add_connection(
            node, node_right, connection=qconn2, label="quantum2")

        # Forward qconn directly to quantum memories for right and left inputs:
        node.ports[port_name].forward_input(node.qmemory.ports["qin0"])  # R input 1
        
        node.ports[port_name2].forward_input(node.qmemory.ports["qin2"])  # R input 2
        
        node_right.ports[port_r_name].forward_input(node_right.qmemory.ports["qin1"])  # L input 1
        
        node_right.ports[port_r_name2].forward_input(node_right.qmemory.ports["qin3"])  # L input 2
        '''
...
def setup_repeater_protocol(network,epsilon, purify="filter"):
    ...
    subprotocol4 = CorrectProtocol(nodes[-1], len(nodes), purify=purify)
...
def setup_datacollector(network, protocol):
    ...
    def calc_fidelity(evexpr):
        position_a=2 if 'Distil' in str([k for k in ConstrainedMapView(protocol.subprotocols,[ValueConstraint(lambda x: 'purify' in x.name)]).values()]) else final_firstpos
        qubit_a, = nodes[0].qmemory.peek([position_a])
        position_b=3 if 'Distil' in str([k for k in ConstrainedMapView(protocol.subprotocols,[ValueConstraint(lambda x: 'purify' in x.name)]).values()]) else final_secpos
        qubit_b, = nodes[-1].qmemory.peek([position_b])
        fidelity =ns.qubits.fidelity([qubit_b, qubit_a], ks.b00, squared=True)
        #print(fidelity)
        protocol.send_signal(Signals.WAITING)
...
def run_simulation(epsilon, num_nodes, node_distance, num_iters, purify):
    ...
    protocol = setup_repeater_protocol(network,epsilon=epsilon, purify=purify)
...
#run_simulation(0, 0.9, conf['num_nodes'], conf['node_distance'], conf['num_iters'])

'''run_simulation(randomval=gauss(1,0.01),
                                    epsilon=0.9,
                                    num_nodes=conf['num_nodes'],
                                    node_distance=conf['node_distance'],
                                    num_iters=10)

run_simulation(randomval=gauss(1,0.2),
...
def create_plot1():
    ...
    ax.fill_between(range(0, 51), mean_fid_distill - std_fid_distill, mean_fid_distill + std_fid_distill,
                    color='blue', alpha=0.2)    
    ax.axhline(mean_fid_raw, color='orange', label=f'Mean fidelity (raw): {mean_fid_raw:.3f}')
    ax.fill_between(range(0, 51), mean_fid_raw - std_fid_raw, mean_fid_raw + std_fid_raw,
    ...
    plt.savefig('Distil_comparison.pdf', format='pdf', dpi=300)
    plt.show()
